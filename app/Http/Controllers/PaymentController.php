...
class PaymentController extends Controller
{
     ...
     */
    public function verifyPayment(Request $request, $id, BonusService $bonusService): JsonResponse // 2. INJECT SERVICE DI SINI
    {
        $request->validate([
        ...
        if ($request->action === 'approve') {
            ...
            // Jalankan seluruh proses aktivasi dan pembagian bonus di dalam DB Transaction safety
            \Illuminate\Support\Facades\DB::transaction(function () use ($payment, $request, $bonusService) {
                // a. Ubah status pembayaran menjadi approved
                $payment->update(['status' => 'approved', 'notes' => $request->notes]);

                // b. AKTIFKAN AKUN USER JADI ACTIVE
                $payment->user->update(['status' => 'active']);

                // c. TEMBAKKAN BONUS JARINGAN MATAHARIsecara otomatis!
                $bonusService->distribute($payment->user_id);
            });

            return response()->json([
                'success' => true,
                'message' => 'Pembayaran disetujui. Akun agen "' . $payment->user->name . '" aktif dan bonus jaringan berhasil dibagikan ke seluruh upline.'
            ]);
