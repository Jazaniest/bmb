...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xxs overflow-hidden">
        <div class="p-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
            ...
            <p class="text-xxs text-slate-400">Pastikan transfer manual ke rekening agen sukses dilakukan sebelum menekan Setuju.</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 text-xs text-slate-600">
                    ...
                    <tr class="hover:bg-slate-50/40 transition-colors">
                        ...
                        <td class="py-4 px-6">
                            <div class="text-base font-black text-slate-800">Rp {{ number_format($wd['amount'], 0, ',', '.') }}</div>
                            <span class="inline-block text-[10px] font-bold text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded mt-0.5 uppercase">Biaya Admin: Rp 0</span>
                        </td>
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ url('/admin/withdrawals/'.$wd['id'].'/approve') }}" method="POST" onsubmit="return confirm('Apakah Anda sudah benar-benar mentransfer dana sebesar Rp {{ number_format($wd['amount'], 0, ',', '.') }} ke rekening tujuan?')">
                                    @csrf
                                ...
                                <form action="{{ url('/admin/withdrawals/'.$wd['id'].'/reject') }}" method="POST" onsubmit="return confirm('Tolak penarikan dana ini? Saldo akan dikembalikan penuh ke dompet agen.')">
                                    @csrf
